...
// Tu codigo acá:
//_________________CLAse 1____________________
/*
1- Construccion del DOM = Document Object Model
es una representacion en un obj 
de la pagina HTML parseada

2- Construccion del CSSDOM = 
es un obj representando los estilos
asociados al DOM

3- Ejecucion de JS = 
JS bloquea el parseo del DOM, cuando el parse llega 
a un tag <script> frena ese recurso y se ejecuta.
para evitarlo = 
* en la etiqueta script se le agrega el async
* se usa la etiqueta abajo del body
*se usa un archivo externo

4- Creacion del Render Tree = 
resder = mostrar en pantalla
se deja solo los elementos visibles

5- Generacion del Layout = 
es lo que toma el tamanio del viewport, cree da el contexto
necesario para calcular los estilos que depende de el

6- Paiting =
la parte visible de la pagina se convierte en pixeles
qeu se muestran en la pantalla
El tiempo de pintado depende del tamanio del DOM, como los estilos 
que se le aplican
...
___________________DOM API____________________________
...
modificar la estructura: agregar, modificar o eliminar
elementod HTML o atributos.

SELECTORE = Los selectores nos permitiran
buscar y recuperar un
elemento del DOM.

~ EventTarget = es una interfaz implementada por los objetos
que pueden recibir eventos y pueden tener escuchadores
para los objetos.

~ Node = es una interfaz en la cuál un número de objetos de
tipo DOM API heredan. Esta interfaz permite que esos
objetos sean tratados similarmente.
...
const divs = document.getElementsByClassName('divClass');[<div class=divClass>, <div class=divClass>]

const div = document.getElementById('divId'); <div id=divID>

const div = document.querySelector('.divId');<div class=divID>

const divs = document.querySelectorAll('.divId');[<div class=divClass>, <div class=divClass>]

let myDiv = document.createElement('div') 'p'/'div'/etc

myDiv.innerText // le agrego un texto

myDiv.appendChild()

myDiv.addEventListener('evento', 'reaccion')

myDiv.addEventListener('click', function(event){
  console.log(event.target) // <div>
  event.target.style.backgound = 'green';
})



////////////////////////////////////////////////////
__________________Event___________________________

Un evento es una señal que algo sucedió.
Todos los nodos del DOM pueden generar estas señales.
...
document.querySelector('#createdBy').innerHTML += ' Nuria'
...
//    3) Crear un elemento 'span' y asignárselo a una variable denominada 'toDoShell'
...
function buildToDo(toDo, index) {
    ...
    toDoText.addEventListener('click', completeToDo) //estaba pasando el cb llamandola
...
function displayToDos() {
    ...
    buildToDos(toDoItems).map((e) => {
        toDoContainer.innerHTML += e;
    })
}
...
function addToDo() {
    // Tu código acá:
    let myToDo = new ToDo('toDoInput');
    toDoItems = [...toDoItems, myToDo];
    toDoInput.value = '';
    displayToDos();
}
